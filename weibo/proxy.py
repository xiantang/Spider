...
# conn = pymysql.connect(host='localhost', user='root', passwd='123456', db='zhaobiao', port=3306, charset='utf8')
# print('链接成功')
...
Cookie:__jsluid=3f8b631de5544d404eb64a83577b457e; gr_user_id=fab8087e-e1b2-4286-b75d-476cfe078b66; UM_distinctid=161c794045e1b2-00fc3893a398bc-3a7f0e5a-100200-161c794045f74; qlm_username=15868759135; qlm_password=E8mmpu8fB87EC88jfj3up3BRp7oURjou; rem_login=1; seo_refUrl="https://www.baidu.com/link?url=FsipNhoi6Han8Bkuab63iDbL0i46cIytjoXKkAe5xvw7-zW_DZ7ie5TVjweCXnJl&wd=&eqid=c51c146600008c6d000000065a939528 

"; seo_curUrl="http://www.qianlima.com/common/area.jsp 

"; seo_intime="2018-02-26 13:03:51"; Hm_lvt_d7682ab43891c68a00de46e9ce5b76aa=1519655630; Hm_lvt_0a38bdb0467f2ce847386f381ff6c0e8=1519621457,1519651654,1519733653,1519794622; Hm_lvt_5dc1b78c0ab996bd6536c3a37f9ceda7=1519622402,1519651654,1519733653,1519794622; gr_session_id_83e3b26ab9124002bae03256fc549065=606de9b4-ec5a-44ca-986b-1b96602de6e9; qlmll_his=",78986517,78986382,78688477,78688488,78688496,78772645,78773957,78774308,78774321,78774968,"; CNZZDATA1848524=cnzz_eid%3D286386678-1519468056-null%26ntime%3D1519797041; __jsl_clearance=1519798422.034|0|X2zCD9ltDnhxMpj9K1aUZ%2BoJ4zw%3D; Hm_lpvt_0a38bdb0467f2ce847386f381ff6c0e8=1519798453; Hm_lpvt_5dc1b78c0ab996bd6536c3a37f9ceda7=1519798453
...
User-Agent:Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/64.0.3282.119 Safari/537.36'''
    ...
    def __init__(self):
        with open('zhaobiao(1).csv', 'a', encoding='utf-8') as f:
            f_csv = csv.writer(f)
    ...
    def analysis_list_html(self, count, i=1):
        ...
        if '黑龙江省招标网' in selector.xpath('//title/text()'):
            ...
            sel = selector.xpath(
                '//table[@width="100%"]/tr')  # /html/body/table[8]/tbody/tr/td[1]/table[4]/tbody/tr[2]/td/table/tbody/tr[1]
            urls = []
            tms = []
            for item in sel:
                url = item.xpath('./td[2]/a[@class="list"]/@href')
                tm = item.xpath('./td[3]/text()')
    ...
    def analysis_content_html(self, url):
        ...
        if title == []:
            ...
            Coarse_content = ''.join(re.findall(r"<div> <div> <div>(.*?)</div> ", html))
    ...
    def write_to_excel(self, items, tm, url):
        with open('zhaobiao.csv', 'a', encoding='utf-8') as f:
            ...
            if items == None:
                ...
                #     if len(items[2]) == 0:
                #         return;
                #     sql = 'INSERT INTO zhaobiao (title,dtime,content,url) VALUES (%s,%s,%s,%s)'
                #     print("tm = ", tm)
                #     cur.execute(sql, (
                #         items[0], tm, items[2], url))
                #     # 提交sql语句
                #     conn.commit()
                #     print('存入成功')
...
if __name__ == '__main__':
    ...
    s = Spider()
    for i in range(1, 40):
        (urls, tms) = s.analysis_list_html(i)
        if urls == None:
            continue
        else:
            # for url in urls:
